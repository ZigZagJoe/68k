...
|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
| first code run on the machine (that is not bootlaoder...)
_start:
	cli
...
_swap_task:
	cli
	tas.b (_swap_in_progress)		    | ensure we are not swapping already
    bne _ret_swap
...
_got_free:
    ...
    move.l %a2, %d0                     | return pointer in %d0
...
task_count:           .space 2
_swap_in_progress:    .space 1
_user_yielded:        .space 1
